...
*/
    ...
    int RandNeighbor(int id){
        ...
        int firstIdNeighbor = mazeCells[neighborsList[id][0]].id;
        int cpt=0;
        //cout<<firstIdNeighbor<<endl;
        for (int i = 0; i < neighborsList[id].size(); i++) {
            //cout<<"salut"<<endl;
            if (mazeCells[neighborsList[id][i]].id == firstIdNeighbor) {
                //cout<<"cpt :"<<cpt<<endl;
                cpt++;
                if(cpt==neighborsList[id].size()){return id_v;} 
            }
        }
