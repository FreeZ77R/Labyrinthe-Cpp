...
pair<int,int> Graph::getXY(int id1, int id2, int nbRows, int nbColumns, int window_w, int window_h) {
	...
	for (int i = 0; i < nbRows; i++) {
		for (int j = 0; j < nbColumns; j++) {
			...
			if (id2 == i * nbColumns + j) {
			    xy = std::make_pair((window_w / nbColumns) * j, (window_h / nbRows) * i);
			    return xy;
			}
...
// Affiche sur la sortie standard les coordonées de chaque cellules du labyrinthe 
...
// Affiche sur la sortie standard les voisins de chaque cellules basé sur les id 
...
// Affiche sur la sortie standard entre qu'elles cellules il y a des murs basé sur les id
...
// Affiche sur la sortie standard la position approximative des murs sur le terminal
...
// Affiche sur la sortie standard les cellules visités et non visités
...
// Affiche sur la sortie standard les identifiants de chaque cellules
void Graph::printId(){
